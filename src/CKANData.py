...
class CKANRecord:
    ...
    def setDestRecord(self, destRecord):
        ...
        if self.destRecord.origin != constants.DATA_SOURCE.DEST:
            msg = (
                f'the dest record provided has a type of {self.destRecord.origin}'
                  "needs to be a 'DEST' type"
            )
    ...
    def getComparableStructUsedForAddUpdate(self, dataCache, operationType, destRecord=None):
        ...
        if methodName not in self.operations:
            ...
            if self.origin != constants.DATA_SOURCE.SRC:
                ...
                if destRecord.origin != constants.DATA_SOURCE.DEST and operationType == constants.UPDATE_TYPES.UPDATE:
    ...
    def applyCustomTransformations(self, applicationType, customTransformationConfig=None):
        """
        ...
        if methodName not in self.operations:
            ...
            if customTransformationConfig:
                ...
                customTransformerMethodNames = [customTransDict[constants.CUSTOM_UPDATE_METHOD_NAME] for customTransDict in customTransformationConfig]
